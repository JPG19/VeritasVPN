...
func (m *MockBTCPayProvider) CreateInvoice(accountID, tier string, amountUSD float64) (string, string, error) {
	id, err := randomID(16)
	if err != nil {
		return "", "", err
	}
...
type InvoiceCreator interface {
	CreateInvoice(accountID, tier string, amountUSD float64) (invoiceID, checkoutURL string, err error)
}
